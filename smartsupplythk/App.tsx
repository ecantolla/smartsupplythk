...
import { ProductCalculatedData, AppView } from './types';
...
const App: React.FC = () => {
  ...
  const handleFileUploaded = useCallback(async (file: File) => {
    ...
    // Pass the number of periods to the processing function
    ...
    setIsLoading(false);
  }, [numPeriods]); // Add numPeriods to dependency array
  ...
  const filteredResults = useMemo(() => {
    ...
    return calculationResults.filter(product => {
      const skuMatch = skuSearchTerms.length === 0 
        ? true 
        : skuSearchTerms.some(term => product['ID_Producto / SKU'].toLowerCase().includes(term));

      const nameMatch = nameSearchTerms.length === 0
        ? true
          ...
          <div className="flex flex-col items-center gap-8">
            ...
            {error && (
              <div className="mt-6 p-4 bg-red-100 border border-red-400 text-red-700 rounded-md w-full max-w-3xl whitespace-pre-wrap">
                <h3 className="font-bold text-lg mb-2">Error{currentFileName ? ` (Archivo: ${currentFileName})`: ''}:</h3>
                <p>{error}</p>
              </div>
            )}
          </div>
        ...
        {view === 'results' && calculationResults && (
          <div>
            <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center mb-6">
              <h2 className="text-2xl lg:text-3xl font-semibold text-slate-700 mb-4 sm:mb-0">Resultados del Cálculo de Abastecimiento</h2>
            ...
            {currentFileName && <p className="text-sm text-slate-600 mb-4">Mostrando resultados para el archivo: <strong>{currentFileName}</strong> (Análisis de {numPeriods} semanas)</p>}
             {error && (
